...
const (
	...
	PDPJobStatusCompensated        PDPJobStatus = "compensated"         // Main record deleted after PDP failure
	PDPJobStatusCompensationFailed PDPJobStatus = "compensation_failed" // Both PDP call and compensation failed
)
...
type PDPJob struct {
	...
	SchemaID       *string        `gorm:"type:varchar(255)" json:"schema_id,omitempty"`      // For policy metadata jobs - also used for compensation
	ApplicationID  *string        `gorm:"type:varchar(255)" json:"application_id,omitempty"` // For allow list jobs - also used for compensation
