...
exchange_credentials_for_token() {
    local api_key=$1
    local api_secret=$2
    local scope=${3:-"gov-dx-api"}
    
    print_substep "1" "Exchanging credentials for Asgardeo token"
    
    local exchange_data=$(cat <<EOF
{
    "apiKey": "$api_key",
...
EOF
    ...
    local response=$(make_post_request "$API_BASE_URL/auth/exchange" "$exchange_data")
    local access_token=$(extract_json_field "$response" "access_token")
    local consumer_id=$(extract_json_field "$response" "consumerId")
    
    if [ -n "$access_token" ] && [ "$access_token" != "null" ]; then
        print_success "Asgardeo token obtained"
        echo "ACCESS_TOKEN:$access_token"
        echo "CONSUMER_ID:$consumer_id"
    else
        print_warning "Token exchange failed (expected if Asgardeo not configured)"
        echo "Response: $response"
        # Return mock token for testing
        local mock_token="mock-asgardeo-token-$(date +%s)"
        echo "ACCESS_TOKEN:$mock_token"
        echo "CONSUMER_ID:$consumer_id"
    fi
}
...
validate_token() {
    ...
    print_substep "2" "Validating Asgardeo token"
...
EOF
    ...
    local response=$(make_post_request "$API_BASE_URL/auth/validate" "$validation_data")
    local is_valid=$(extract_json_field "$response" "valid")
    local consumer_id=$(extract_json_field "$response" "consumerId")
    local error=$(extract_json_field "$response" "error")
