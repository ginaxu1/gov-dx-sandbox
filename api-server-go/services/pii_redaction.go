...
type PIIRedactionService struct {
	// Pattern to match fields ending with Id or _id (case-insensitive)
...
// RedactData removes all fields ending with Id or _id from the data
...
func (s *PIIRedactionService) redactMap(m map[string]interface{}) map[string]interface{} {
	...
	for key, value := range m {
		// Check if the key ends with Id or _id (case-insensitive)
...
func (s *PIIRedactionService) extractCitizenID(data interface{}) string {
	...
	case map[string]interface{}:
		...
		for _, value := range v {
			if id := s.extractCitizenID(value); id != "" {
				return id
			}
		}
	...
	case []interface{}:
		...
		for _, item := range v {
			if id := s.extractCitizenID(item); id != "" {
				return id
			}
		}
	}
