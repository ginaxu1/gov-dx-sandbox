...
func (h *V1Handler) SetupV1Routes(mux *http.ServeMux) {
	// Provider routes
	mux.Handle("/api/v1/providers", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleProviders)))
	mux.Handle("/api/v1/providers/", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleProviders)))

	// Schema routes
	mux.Handle("/api/v1/schemas", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleSchemas)))
	mux.Handle("/api/v1/schemas/", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleSchemas)))

	// SchemaSubmission routes
	mux.Handle("/api/v1/schema-submissions", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleSchemaSubmissions)))
	mux.Handle("/api/v1/schema-submissions/", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleSchemaSubmissions)))

	// Consumer routes
	mux.Handle("/api/v1/consumers", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleConsumers)))
	mux.Handle("/api/v1/consumers/", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleConsumers)))

	// Application routes
	mux.Handle("/api/v1/applications", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleApplications)))
	mux.Handle("/api/v1/applications/", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleApplications)))

	// ApplicationSubmission routes
	mux.Handle("/api/v1/application-submissions", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleApplicationSubmissions)))
	mux.Handle("/api/v1/application-submissions/", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleApplicationSubmissions)))

	// Entity routes
	mux.Handle("/api/v1/entities", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleEntities)))
	mux.Handle("/api/v1/entities/", middleware.PanicRecoveryMiddleware(http.HandlerFunc(h.handleEntities)))
}
