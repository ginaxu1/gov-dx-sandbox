...
func TestConsentEngine_UpdateConsent(t *testing.T) {
	...
	if updatedRecord.Status != StatusApproved {
		t.Errorf("Expected status=StatusApproved, got %v", updatedRecord.Status)
	}
...
func TestConsentEngine_DefaultRecord(t *testing.T) {
	...
	if defaultRecord.Status != StatusPending {
		t.Errorf("Expected Status=StatusPending, got %v", defaultRecord.Status)
	}
