...
func NewDatabaseConfig() *DatabaseConfig {
	...
	return &DatabaseConfig{
		Host:            utils.GetEnvOrDefault("CHOREO_DB_CE_HOSTNAME", "localhost"),
		Port:            utils.GetEnvOrDefault("CHOREO_DB_CE_PORT", "5432"),
		Username:        utils.GetEnvOrDefault("CHOREO_DB_CE_USERNAME", "postgres"),
		Password:        utils.GetEnvOrDefault("CHOREO_DB_CE_PASSWORD", "password"),
		Database:        utils.GetEnvOrDefault("CHOREO_DB_CE_DATABASENAME", "consent_engine"),
		SSLMode:         utils.GetEnvOrDefault("DB_SSLMODE", "require"),
...
func parseIntOrDefault(key string, defaultValue int) int {
	if value := utils.GetEnvOrDefault(key, ""); value != "" {
...
func parseDurationOrDefault(key, defaultValue string) time.Duration {
	if value := utils.GetEnvOrDefault(key, defaultValue); value != "" {
