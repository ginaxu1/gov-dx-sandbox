...
func TestLogGeneralizedAudit(t *testing.T) {
	...
	t.Run("ExplicitTraceID", func(t *testing.T) {
		...
		mu.Lock()
		defer mu.Unlock()
		if receivedRequest.TraceID == nil || *receivedRequest.TraceID != traceID {
			got := ""
			if receivedRequest.TraceID != nil {
				got = *receivedRequest.TraceID
			}
