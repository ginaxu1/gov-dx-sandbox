...
PDP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/decide 2>/dev/null || echo "000")
if [ "$PDP_STATUS" = "400" ] || [ "$PDP_STATUS" = "200" ]; then
...
CE_STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8081/consent 2>/dev/null || echo "000")
if [ "$CE_STATUS" = "301" ] || [ "$CE_STATUS" = "200" ] || [ "$CE_STATUS" = "400" ]; then
